...
class Pattern2Html 
{
    ...
    public function __construct($pattern_file, $titles = true, $numbers = true)  
    {
        if (!$pattern_file) {
            throw new Exception("Файл шаблона описания не определен");
        }
        $full_path = PATTERNS . DS . $pattern_file;
        if (!file_exists($full_path)) {
            throw new Exception("Файл шаблона описания не найден");
        }
        $this->pattern_DOMDoc = new DOMDocument();
        $this->pattern_DOMDoc->preserveWhiteSpace = false;
        $this->pattern_DOMDoc->formatOutput = true;
        $this->pattern_DOMDoc->load($full_path);
