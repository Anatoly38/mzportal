<?php
/**
* @version		$Id: register_onmk.php,v 1.0 2010/05/27 11:13:51 shameev Exp $
...
require_once ( 'model' . DS . 'adapter_onmk_query.php' );
require_once ( 'model' . DS . 'patient_onmk_save.php' );
require_once ( 'model' . DS . 'patient_onmk_delete.php' );
require_once ( 'views' . DS . 'register_onmk_list.php' );
require_once ( 'views' . DS . 'patient_onmk.php' );

class RegisterOnmk extends Component
{
    ...
    {
        if (!$this->oid[0]) {
            $s = new PatientOnmkSave();
            $s->insert_data();
        } 
        else {
            $s = new PatientOnmkSave($this->oid[0]);
            $s->update_data();
        }
        $this->view_list();
    }
    ...
    {
        $i = new PatientOnmkDelete($this->oid);
        $this->view_list();
    }

    protected function view_list()
    {
        $list = new RegisterOnmkList();
        self::set_title('Список пациентов с острым нарушением мозгового кровообращения');
