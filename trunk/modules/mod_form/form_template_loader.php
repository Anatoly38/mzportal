<?php
/** 
* @version		$Id: form_template_loader.php,v 1.1 2011/06/22 23:50:30 shameev Exp $
...
/*     private function set_lpu($tags)
    {
        if (!$tags) {
            return true;
        }
        require_once('select_lpu.php');
        for ($i = $tags->length; --$i >= 0;) {
            $name = $tags->item($i)->getAttribute('name');
            $lpu = new SelectLpu($name);
            $new_node = $this->form_xml->importNode($lpu->ensemble(), true);
            $tags->item($i)->parentNode->replaceChild($new_node, $tags->item($i));
        }
        return true;
    } */
